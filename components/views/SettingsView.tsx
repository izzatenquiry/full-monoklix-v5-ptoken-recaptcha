...
import {
    EyeIcon, EyeOffIcon, RefreshCwIcon, SparklesIcon, KeyIcon, ShieldCheckIcon, ClipboardIcon, SendIcon, CheckCircleIcon, AlertTriangleIcon, DatabaseIcon, UploadIcon
} from '../Icons';
...
const getTabs = (): Tab<SettingsTabId>[] => [
    { id: 'profile', label: 'Profil & Activation' },
];
...
const ActivationPanel: React.FC<{currentUser: User, onUserUpdate: (u: User) => void}> = ({ currentUser, onUserUpdate }) => {
    ...
    const handleSync = async () => {
        ...
        try {
            ...
            if (resYa29.success && resRec.success) {
                ...
                // Kosongkan reCAPTCHA sebab dia tak boleh recycle
                setRecaptchaInput('');
            } else {
                setSaveStatus('error');
            }
    ...
    const copyBridgeScript = () => {
        ...
        const snippet = `
(async () => {
  console.log("%c 🚀 MONOklix Quantum Bridge V6 ", "background: #4A6CF7; color: white; padding: 10px; border-radius: 8px; font-weight: bold;");
  try {
    console.log("🔑 Mencari Access Token (ya29)...");
    const sessionRes = await fetch('https://labs.google/fx/api/auth/session');
    const sessionData = await sessionRes.json();
    
    // Logic cari ya29 yang lebih robust
    const ya29 = sessionData.accessToken || 
                 sessionData.token || 
                 Object.values(sessionData).find(v => typeof v === 'string' && v.startsWith('ya29.'));

    console.log("🔐 Menjana reCAPTCHA (Action: PINHOLE_GENERATE)...");
    const recToken = await grecaptcha.enterprise.execute('${siteKey}', {action: 'PINHOLE_GENERATE'});

    console.log("%c ✅ DATA BERJAYA EKSTRAK ", "color: #A05BFF; font-weight: bold; font-size: 14px;");
    ...
    if(!ya29) {
        alert("⚠️ Ralat: YA29_TOKEN tidak dijumpai. Pastikan anda sudah login akaun Google AI.");
    } else {
        alert("✅ Berjaya! Sila salin YA29_TOKEN dan RECAPTCHA_TOKEN dari Console (F12) ke MONOklix.");
    }
  } catch (e) {
    alert("❌ Gagal: Pastikan anda berada di tab FLOW Google Labs dan Console dibuka.");
    console.error(e);
  }
})();`.trim();
        
        navigator.clipboard.writeText(snippet);
        alert("Skrip Quantum Bridge disalin! Sila buka Google Labs dan paste dlm Console.");
    };
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-brand-start/20 to-brand-end/10 border border-brand-start/30 p-6 rounded-3xl relative overflow-hidden shadow-glow">
                <div className="relative z-10">
                    ...
                    <p className="text-sm text-neutral-400 mt-1">Sistem ini diperlukan untuk membolehkan Proxy MONOklix menggunakan kuota akaun personal anda secara selamat.</p>
            ...
            <div className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-5">
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <input 
                            ...
                            placeholder="Tampal token 03AFcY... di sini"
                            className="w-full bg-black/40 border border-white/10 rounded-xl p-4 text-white font-mono text-xs focus:ring-1 focus:ring-brand-start outline-none transition-all" 
                        />
                ...
                <div className="pt-4">
                    ...
                    {saveStatus === 'success' && <p className="text-center text-[10px] text-green-500 mt-2 font-bold animate-pulse uppercase">Handshake Berjaya! Anda sedia menjana kandungan.</p>}
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="h-full flex flex-col max-w-4xl mx-auto px-4">
            <div className="mb-8">
                ...
                <p className="text-neutral-500 mt-1">Urus profil dan konfigurasi Quantum Bridge.</p>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pb-20">
                <div className="space-y-8 animate-zoomIn">
                    {/* Activation Panel Sentiasa di Atas */}
                    ...
                    <div className="space-y-6">
                        <div className="bg-white/5 border border-white/10 p-8 rounded-3xl">
                            ...
                            }} className="bg-white text-black px-8 py-3 rounded-xl font-bold text-sm hover:scale-105 transition-all">Save Profile</button>
                        </div>
                        
                        <div className="bg-white/5 border border-white/10 p-8 rounded-3xl">
                            <div className="flex items-center gap-3 mb-4">
                                <DatabaseIcon className="w-6 h-6 text-red-500" />
                                <h3 className="text-xl font-bold text-white tracking-tight">Media Cache Engine</h3>
                            </div>
                            <p className="text-sm text-neutral-500 mb-6">Padam cache video jika galeri tempatan mengalami masalah loading.</p>
                            <button 
                                onClick={() => {
